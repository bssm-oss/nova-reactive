...
- [Spring Data 스타일 Repository (`nova-spring-data`)](#spring-data-%EC%8A%A4%ED%83%80%EC%9D%BC-repository-nova-spring-data)
...
// build.gradle.kts
dependencies {
    implementation("io.github.bssm-oss:nova-core:1.0.0")

    // 사용할 데이터베이스 다이얼렉트만 추가합니다
    implementation("io.github.bssm-oss:nova-dialect-postgresql:1.0.0")
}
```
...
Nova `1.0.0`은 Maven Central에 GA로 배포되어 있습니다.
...
> **Snapshot 사용**: 다음 dev 빌드(`1.0.1-SNAPSHOT` 등)는 Central snapshots 저장소에서 받을 수 있습니다.
>
> ```kotlin
> repositories {
>     mavenCentral()
...
```kotlin
// build.gradle.kts
implementation("io.github.bssm-oss:nova-metrics-micrometer:1.0.0")
...
// build.gradle.kts
dependencies {
    implementation("io.github.bssm-oss:nova-spring-boot-starter:1.0.0")
    implementation("io.github.bssm-oss:nova-dialect-postgresql:1.0.0")
...
JPA/Spring Data 사용자에게 익숙한 `interface ... extends ReactiveCrudRepository<T, ID>` 패턴을 별도 의존(`io.github.bssm-oss:nova-spring-data:1.0.0`)으로 제공합니다. Spring Data Commons에는 의존하지 않으며 Spring Framework `spring-context`만 사용합니다.
...
└── nova-project/                                       # spring-boot 스타일 모듈 컨테이너
    ...
    │       ├── annotation/                             # @Entity, @Id, @Column, @GeneratedValue, @CreatedAt, @UpdatedAt, @SoftDelete, @Version, @PrePersist/Update/Remove, @PostLoad, @Embeddable, @Embedded, @Index, @UniqueConstraint, @ManyToOne, @OneToMany, @JoinColumn, @Enumerated
    ...
    │       ├── fetch/                                  # FetchGroup DSL + AnnotationFetchGroupBuilder
    ...
    │       ├── query/                                  # Criteria, Predicate, QuerySpec, Sort, Pageable, PageRequest, Page, Slice, Updater, Projection, NativeQuery, Cursor, LockMode, AggregateSpec/Aggregation/AggregateRow
    ...
    ├── nova-spring-data/                               # io.nova.spring.data — ReactiveCrudRepository, @EnableNovaRepositories, JDK proxy 위임
...
> Maven 좌표는 평탄(`io.github.bssm-oss:nova-core`)하게 유지됩니다 — Gradle 경로만 `:nova-project:...`로 중첩.
...
- [x] FetchGroup DSL — annotation 없이 명시적 batch child fetch (N+1 회피)
