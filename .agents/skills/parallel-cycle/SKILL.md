...
**스코프 분리 → 설계-우선 병렬 개발(opus 설계→sonnet 구현) → (위험도 차등 병렬 리뷰 ↔ 보완)\* → 수렴-즉시 스트리밍 병합 → (E2E 전용 opus agent ↔ 보완)\* → PR 생성·머지 → 버전 태그 → 배포 백그라운드 레인 + 다음 cycle 진행 → 공개 확인**
...
4. 태그 push 직후 배포 확인을 **별도 비동기 레인**으로 전환한다. `.github/workflows/release.yml`의 tag workflow, Maven Central publish, Central Portal 동기화, GitHub Release 공개는 background watcher 또는 별도 task가 주기적으로 확인한다. foreground 구현 레인을 `gh run watch` 출력 대기로 점유하지 않는다.
5. 배포 레인이 기다리는 동안에는 다음 disjoint cycle의 scope 확정·설계·구현·리뷰를 진행할 수 있다. 단 이전 배포가 성공/실패로 확정되기 전에는 다음 release tag를 push하지 않는다.
6. Central Portal deployment와 GitHub Release가 실제 공개 상태인지 확인한다. credentials/권한/CI 실패로 확인할 수 없으면 배포 완료로 표시하지 말고 blocker로 보고하고, 개발 레인과 분리해 추적한다.
7. 배포 확인 전에는 해당 cycle을 완료로 선언하지 않는다. 배포 대기 때문에 다음 cycle 개발을 막지는 않되, 해당 cycle의 배포 추적 상태는 명시적으로 유지한다.
...
- PR 머지 뒤 개발 worktree/브랜치는 정리할 수 있지만, 별도 배포 추적 항목은 Maven Central과 GitHub Release 공개 확인까지 닫지 않는다. 외부 push 권한이나 release secret이 없으면 정확한 blocker를 사용자에게 보고하고 배포 레인은 열린 상태로 둔다.
