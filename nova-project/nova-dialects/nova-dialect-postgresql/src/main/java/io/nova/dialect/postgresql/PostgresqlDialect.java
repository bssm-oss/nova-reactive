...
public final class PostgresqlDialect implements Dialect {
    private final BindMarkerStrategy bindMarkers = index -> {
        if (index < 1) {
            throw new IllegalArgumentException("bind marker index must be positive");
        }
        return "$" + index;
    };
    ...
    public String renderCall(String procedureName, int parameterCount) {
        ...
        // bind marker contract는 1-based다. 기본 renderCall도 같은 계약을 따르지만, 이 구현은 PostgreSQL
        // FUNCTION 문법을 사용하므로 여기서도 i+1로 명시해 다른 렌더링 경로($1부터 시작)와 정합시킨다.
