...
// build.gradle.kts
dependencies {
    implementation("io.github.bssm-oss:nova-spring-boot-starter:2.12.0")
...
The familiar `interface ... extends ReactiveCrudRepository<T, ID>` pattern is available as a separate dependency (`io.github.bssm-oss:nova-spring-data:2.12.0`). It depends only on Spring Framework's `spring-context` — not on Spring Data Commons.
