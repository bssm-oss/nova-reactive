...
Bind each declared slot through its declared name or its 1-based position. A named declaration may use either address form, but not both; rebinding through the same form overwrites the prior value. Blank/unknown names, unknown positions, duplicate declared names, and cross-address rebinding fail before native work. Non-null values must be assignable to the declared Java type (primitive declarations accept their wrapper); Nova does not coerce values or supply default arguments. A null value is accepted and, when the declaration has a Java type, is passed as a typed R2DBC null binding; an undeclared type retains the normal executor fallback.
