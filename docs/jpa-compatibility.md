...
| Scalar types | ✅ | `UUID`, `Float`, `Short`, `BigDecimal`, `BigInteger`(driver-permitting), … — driver-verified. `BigDecimal` DDL preserves declared `@Column(precision, scale)` on scalar, id/FK, join-table, and collection-table storage; MySQL/MariaDB reject an unspecified or scale-only shape. |
