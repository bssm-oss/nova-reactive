...
| JPQL `setFirstResult` / `setMaxResults` | ✅ | Negative values are rejected; zero max results completes without querying. Entity SELECTs use the dialect renderer, including offset-only requests as `Integer.MAX_VALUE` limits. Scalar, aggregate, and `SELECT NEW` projections preserve their SQL bindings and apply the window with reactive `skip` / `take` after query execution because arbitrary JPQL SQL has no public dialect pagination hook. This projection path may read skipped rows from the database. |
