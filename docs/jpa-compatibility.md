...
| JPQL `setFirstResult` / `setMaxResults` | ✅ | Negative values are rejected; zero max results completes without querying. Simple entity SELECTs use the dialect renderer, including offset-only requests as `Integer.MAX_VALUE` limits. Filtering-join entity SELECTs page ordered, distinct root IDs reactively before bounded (256-id) hydration chunks. Scalar, aggregate, and `SELECT NEW` projections preserve their SQL bindings and apply the window with reactive `skip` / `take` before result mapping because arbitrary JPQL SQL has no public dialect pagination hook. These client-side paths may read skipped rows from the database. |
