...
| `LockModeType` (`PESSIMISTIC_WRITE`/`READ`, `OPTIMISTIC`, `FORCE_INCREMENT`) | ✅ | `find` / `lock` / `getLockMode` overloads. `getLockMode` is `NONE` for an ordinary managed find and reports the exact alias only after that exact managed instance successfully acquires a lock; detached, failed, or cancelled lock attempts do not change it, and `detach` / `clear` reset it to `NONE`. |
