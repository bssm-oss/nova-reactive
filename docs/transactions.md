...
- `refresh` performs its fresh read without the persistence-session binding while retaining the surrounding Reactor transaction context. With `nova-cache`, that one read bypasses shared entity-cache lookup and population without evicting warm entity or query-cache entries; subsequent ordinary reads retain normal cache behavior.
